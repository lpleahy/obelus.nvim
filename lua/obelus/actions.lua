...
function M.instructions(comments, key)
  ...
  local lines = {
    ...
    "",
    '  { "comment_id": "<id>",',
    '    "action": "resolve" | "reopen" | "needs_response" | "reply" | "move",',
    '    "message": "<text — for needs_response / reply>",',
    '    "line": <number>  // for move: the new line the comment refers to',
    "  }",
    "",
    "- resolve: ONLY when the comment asked for a concrete change AND you fully made it.",
    "  A question, discussion, or request for explanation is NEVER resolved — reply instead.",
    "  When in doubt, reply; the human resolves what they consider settled.",
    ...
    '- reply: answer/leave a note (also for "no change needed") without resolving; put it in "message".',
