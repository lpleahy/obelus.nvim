...
T.it("tag chip: one treatment — bright brand fg from the theme, bold, NO box, both modes", function()
  vim.api.nvim_set_hl(0, "Normal", { bg = 0x1e1e2e })
  local ok, err = pcall(function()
    T.fresh({})
    require("obelus.thread").setup_highlights()
    ...
    local brand = vim.api.nvim_get_hl(0, { name = "ObelusBorder", link = false })
    T.ok(chip.bold, "the chip is bold (no code group is)")
    T.is_nil(chip.bg, "no box — the bubble shows through")
    T.eq(chip.fg, brand.fg, "the chip is the theme brand colour (same as the chrome accent)")
    T.ok(not code.bold, "inline code stays non-bold — bold is the tag's separator")
    local badge = vim.api.nvim_get_hl(0, { name = "ObelusTagBadge", link = false })
    T.ok(badge.bold and badge.bg == nil and badge.fg == chip.fg, "the explorer badge is the identical treatment")
  end)
  vim.api.nvim_set_hl(0, "Normal", {})
  if not ok then
    error(err, 0)
  ...
  require("obelus.thread").setup_highlights()
  local chip = vim.api.nvim_get_hl(0, { name = "ObelusThreadTag", link = false })
  T.is_nil(chip.bg, "identical on transparent themes (fg-only by construction)")
  T.ok(chip.bold, "…and still bold")
end)

T.it(
  "markview tag chip: Obelus_MarkviewPalette7 is the same bold brand fg, no box (twin of ObelusThreadTag)",
  function()
    vim.api.nvim_set_hl(0, "Normal", { bg = 0x1e1e2e })
    local ok, err = pcall(function()
      T.fresh({})
      ...
      local chip = vim.api.nvim_get_hl(0, { name = "ObelusThreadTag", link = false })
      T.ok(pill.bold, "the markview tag chip is bold")
      T.is_nil(pill.bg, "no box — one colour, straight from the theme")
      T.eq(pill.fg, chip.fg, "markview + builtin chips share the one brand recipe")
    end)
    vim.api.nvim_set_hl(0, "Normal", {})
    if not ok then
      error(err, 0)
